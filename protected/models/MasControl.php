...
class MasControl extends CActiveRecord {
...
//-----------------------------------------------------------------------------------------------------------------
	...
	public function save_insert() {
		try{
			...
			$sql = "
			INSERT INTO mas_control_app (ct_type, ct_name, ct_check, ct_order, app_id, createby, createdate, updateby, updatedate)
			VALUES (:type, :name, :check, :order, :app_id, '$createby', now(), '$createby', now())
			";
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_update() {
		try{
			...
			$transaction = $conn->beginTransaction();
			$sql = "UPDATE mas_control_app SET ct_type=:type, ct_name=:name, ct_check=:check, updatedate=now(), updateby='$createby' WHERE ct_id=:id";
			$command = $conn->createCommand($sql);
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_delete() {
		...
		$sql = "update mas_control_app set ct_status=0, updatedate=now(), updateby='$createby' where ct_id = ".$this->id;
