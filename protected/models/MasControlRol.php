...
class MasControlRol extends CActiveRecord {
...
//-----------------------------------------------------------------------------------------------------------------
	...
	public function save_insert() {
		try{
			...
			$sql = "
			INSERT INTO mas_control_rol (ct_type, ct_name, ct_check, ct_order, rol_id, createby, createdate, updateby, updatedate)
			VALUES (:type, :name, :check, :order, :rol_id, '$createby', now(), '$createby', now())
			";
			...
			$command->bindValue(":rol_id", $this->rol_id);
			$command->execute();
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_update() {
		try{
			...
			$transaction = $conn->beginTransaction();
			$sql = "UPDATE mas_control_rol SET ct_type=:type ,ct_name=:name, ct_check=:check, updatedate=now(), updateby='$createby' WHERE ct_id=:id";
			$command = $conn->createCommand($sql);
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_delete() {
		...
		$sql = "update mas_control_rol set ct_status=0, updatedate=now(), updateby='$createby' where ct_id = '".$this->id."'";
