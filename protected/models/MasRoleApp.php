...
class MasRoleApp extends CActiveRecord {
...
//-----------------------------------------------------------------------------------------------------------------
	...
	public function save_insert() {
		try{
			...
			$sql = "
			INSERT INTO mas_role_app (ra_code, app_id, ra_name, ra_description, create_by, create_date, update_by, update_date)
			...
			$sql2 = " SELECT ra_id FROM mas_role_app ORDER BY ra_id DESC LIMIT 1 ";
			$rows = Yii::app()->db->createCommand($sql2)->queryAll();
			foreach ($rows as $dataitem){
				$r1 = MasPositionRol::check_position_rol($dataitem["ra_id"]);
				$r2 = MasLevelRol::check_level_rol($dataitem["ra_id"]);
				$r3 = MasDepartmentRol::check_department_rol($dataitem["ra_id"]);
			}
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_update() {
		try{
			...
			$transaction = $conn->beginTransaction();
			$sql = "UPDATE mas_role_app SET ra_code=:ra_code, app_id=:app_id, ra_name=:ma_name, ra_description=:descri, update_date=now(), update_by='$createby' WHERE ra_id=:id";
			$command = $conn->createCommand($sql);
...
//-----------------------------------------------------------------------------------------------------------------
	public function save_delete() {
		...
		$sql = "update mas_role_app set status=0, update_date=now(), update_by='$createby' where ra_id = '".$this->id."'";
