<?php
    if(isset($_POST['ide'])){
      $header_m = 'แก้ไขข้อมูล ประเภทบุคลากร';

      //***********************************---------  Call Data MasApp BY ID

      $CallGroup = AdminController::CallTypeByID($_POST['ide']);

      //***********************************---------  END 

    }else{
      $header_m = 'เพิ่มข้อมูล ประเภทบุคลากร';
    }
?>
...
<div class="modal-body font-03">
  <div class="example-wrap">
    <div class="form-group">
      <p class="example-title font-03 font-w"> ชื่อ ประเภทบุคลากร </p>
      <input class="form-control form-control-lg font-03 ap_name_e" id="ap_name_e" type="text" value="<?php echo $CallGroup['ut_name'];?>">
    ...
    <div class="form-group">
      ...
      <input class="form-control form-control-lg font-03 ap_des_e" id="ap_des_e" type="text" value="<?php echo $CallGroup['ut_description'];?>">
      <div class="invalid-feedback font-05 des_e_msg" style="display:none;"> กรอกข้อมูลให้ครบถ้วน</div>
    </div>
  </div>
</div>
<div class="modal-footer">
  <button class="btn btn-success font-03 font-w btn-submit-e" id="<?php echo $CallGroup['ut_id'];?>" type="button">แก้ไขข้อมูล</button>
...
<div class="modal-body font-03">
  <div class="example-wrap">
    <div class="form-group">
      <p class="example-title font-03 font-w"> ชื่อ ประเภทบุคลากร </p>
      <input class="form-control form-control-lg font-03 ap_name" id="ap_name" type="text" placeholder="เช่น พนักงานประกันสังคม">
      <div class="invalid-feedback font-05 name_msg" style="display:none;"> กรอกข้อมูลให้ครบถ้วน</div>
    </div>
    <div class="form-group">
      <p class="example-title font-03 font-w"> รายละเอียด (ชื่อภาษาไทย)</p>
      <input class="form-control form-control-lg font-03 ap_des" id="ap_des" type="text" placeholder="เช่น พนักงานประกันสังคม">
  ...
  $(document).ready(function() {
    ...
    $('.btn-submit-e').click(function(){
      ...
      $('.btn-submit-e').attr('disabled',true);
        ...
        //############------------ SEND DATA TO --> Controller = setting / fn = updatetype

        $.ajax({
            type: "POST",
            url: "<?php echo Yii::app()->createAbsoluteUrl("admin/updatetype"); ?>",
            data: {
              packdata : packdata_e,
    ...
    $('.btn-submit').click(function(){
      ...
      //############------------ SEND DATA TO --> Controller = setting / fn = inserttype

      $.ajax({
          type: "POST",
          url: "<?php echo Yii::app()->createAbsoluteUrl("admin/inserttype"); ?>",
          data: {
            packdata : packdata,
            'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>'
          },
