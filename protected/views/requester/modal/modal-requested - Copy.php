		...
		<div class="col-md-12" style="padding-left:15px;">
			...
			<div class="col-sm-12">
				<div class="row">
					...
					<label class="col-sm-6 form-control-label text-right"><span style="color:#6610f2;">วันที่ขอสิทธิ : </span><?php echo Yii::app()->CommonFnc->DateThai($dataitem["req_date"],false); ?></label>
				</div>
			</div>
			<div class="col-sm-12">
				<div class="row"> <?php
										...
										<div class="panel-body" style="background-color:#F2F3F4;"> <?php
											...
											foreach($data2 as $dataitem2) { ?>
												<div class="form-group row" style="background-color: white; margin-bottom:10px;width:100%; border:.5px solid #ddd; padding-bottom:10px">
													<label class="col-sm-3 form-control-label text-left font-w"><?php echo $dataitem2["ma_name"]; ?> : </label>
													<div class="col-sm-9" style="padding-top: 10px;">
														<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem2["ra_name"]; ?>" readonly="readonly">
														<div class="rolecontrol" style="background-color:#F2F3F4; border-bottom-left-radius:10px; border-bottom-right-radius:10px;"> <?php
															$data3=MasRequest::getconrole($dataitem["req_code"],$dataitem2["mod_id"],"");
															$i2 = 0;
															foreach($data3 as $dataitem3) {
																if($i2==0){ echo "<div style='background-color:#F2F3F4; padding:5px'></div>"; }
																if($dataitem3["form_type"]=="text" || $dataitem3["form_type"]=="textarea"){ ?>
																	<div class="form-group row" style="margin-bottom:10px;width:100%;padding-left:10px">
																		<label class="col-sm-4 form-control-label text-left"><?php echo $dataitem3["ct_name"]; ?></label>
																		<div class="col-sm-8">
																			<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem3["form_value"]; ?>" readonly="readonly">
																		</div>
																	</div> <?php
																}else if($dataitem3["form_type"]=="option"){
																	if($dataitem3["form_value"]=="true"){ ?>
																		<div class="form-group row" style="margin-bottom:10px;width:100%;padding-left:10px">
																			<label class="col-sm-4 form-control-label text-left"></label>
																			<div class="col-sm-8">
																				<span>
																					<i class="fa fa-check-circle" aria-hidden="true" style="margin-right:5px;color:#28a745;"></i><?php echo $dataitem3["ct_name"]; ?>
																				</span>
																			</div>
																		</div> <?php
																	}
																}else if($dataitem3["form_type"]=="label"){ ?>
																	<div class="form-group row" style="margin-bottom:10px;width:100%;padding-left:10px">
																		<label class="col-sm-8 form-control-label text-left"><?php echo $dataitem3["form_value"]; ?></label>
																	</div> <?php
																}
																$i2++;
															} ?>
														</div> <?php
														$conmoddata = MasRequest::getconmod($dataitem["req_code"],$dataitem2["mod_id"],"");
														$i = 0;
														$m = 0;
														foreach ($conmoddata as $dataitem4) {
															...
															<div class="form-group row" style="margin-bottom:10px;width:100%;"><?php
																if($dataitem4["form_type"] == "text"){ ?>
																	<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"><?php echo $dataitem4["ct_name"]; ?></label>
																	<div class="col-sm-8">
																		<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem4["form_value"]; ?>" readonly="readonly">
																	</div> <?php
																}else if($dataitem4["form_type"] == "textarea"){ ?>
																	<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"><?php echo $dataitem4["ct_name"]; ?></label>
																	<div class="col-sm-8">
																		<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem4["form_value"]; ?>" readonly="readonly">
																	</div> <?php
																}else if($dataitem4["form_type"] == "option"){
																	if($dataitem4["form_value"]=="true"){ ?>
																		<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"></label>
																		<div class="col-sm-8">
																			<span>
																				<i class="fa fa-check-circle" aria-hidden="true" style="margin-right:5px;color:#28a745;"></i><?php echo $dataitem4["ct_name"]; ?>
																			</span>
																		</div> <?php
																	}
																	$m++;
																}else if($dataitem4["form_type"] == "label"){ ?>
																	<label class="col-sm form-control-label text-left"><?php echo $dataitem4["ct_name"]; ?></label> <?php
																} ?>
															</div> <?php
															$i++;
														} ?>
											...
											$controlapp = MasRequest::getconapp($dataitem["req_code"],"");
											foreach ($controlapp as $dataitem5) { ?>
												<div class="form-group row" style="margin-bottom:10px;width:100%;"><?php
													if($dataitem5["form_type"] == "text"){ ?>
														<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"><?php echo $dataitem5["ct_name"]; ?></label>
														<div class="col-sm-8">
															<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem5["form_value"]; ?>" readonly="readonly">
														</div> <?php
													}else if($dataitem5["form_type"] == "textarea"){ ?>
														<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"><?php echo $dataitem5["ct_name"]; ?></label>
														<div class="col-sm-8">
															<input class="form-control font-03" id="" type="text" value="<?php echo $dataitem5["form_value"]; ?>" readonly="readonly">
														</div> <?php
													}else if($dataitem5["form_type"] == "option"){
														if($dataitem5["form_value"]=="true"){ ?>
															<label class="col-sm-4 form-control-label text-left" style="padding-left:10px"></label>
															<div class="col-sm-8">
																<span>
																	<i class="fa fa-check-circle" aria-hidden="true" style="margin-right:5px;color:#28a745;"></i><?php echo $dataitem5["ct_name"]; ?>
																</span>
															</div> <?php
														}
													}else if($dataitem5["form_type"] == "label"){ ?>
														<label class="col-sm-8 form-control-label text-left"><?php echo $dataitem5["form_value"]; ?></label> <?php
													} ?>
												</div> <?php
											} ?>
										</div>
