...
<div class="pos">
	<div class="bhoechie-tab-content active">
		<div class="col-sm-12 col-xs-12 contact-info-box" style="margin-top:20px;height:500px;overflow:auto;">
			<table id="table_pos-app" class="table table-bordered table-hover table-striped dataTable">
				<thead style="background-color:#AED6F1;">
					<tr>
						...
						<th class="example-title" id="ca" style="width:70px;"> <?php
							$chkmas0 = MasPositionApp::check_stat_all($app);
							...
							<label class="switch">
								<input type="checkbox" id="change_pos" <?php echo $ch; ?> onclick="changeall()">
				...
				<tbody> <?php
					$chkmas = MasPositionApp::check_position_app($app);
					$masterdata = MasPositionApp::search($app);
					...
					foreach ($masterdata as $dataitem) { ?>
						<tr>
							...
							if($dataitem['status'] == 1){
								...
								<label class="switch">
									<input type="checkbox" <?php echo $ch; ?> id="btnsubmitpos<?php echo $dataitem['id']; ?>" data-id="<?php echo $dataitem['id']; ?>" onclick="changestat(<?php echo $dataitem['id']; ?>)">
									<span class="slider round"></span>
								</label>
							</td> 
						</tr> <?php
...
<script type="text/javascript">
	...
	function changestat(id) {
		...
		$.ajax({
			type: "POST",
			url:"<?php echo Yii::app()->createAbsoluteUrl("owner/savechangepos"); ?>",
			data: {
	...
	function changeall() {
		var status = '';
		var btn = "input[id^='btnsubmitpos']";
		
		if($("#change_pos").is(":checked")){
			status = 1;
		...
		$.ajax({
			type: "POST",
			url:"<?php echo Yii::app()->createAbsoluteUrl("owner/saveposappall"); ?>",
			data: {
				'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>',
				'app' : '<?php echo $app; ?>',
				'status' : status,
