...
if($rol==""){ ?>
	...
	<div class="pos-rol">
		<div class="col-sm-12 col-xs-12 contact-info-box" style="margin-top:20px;height:500px;overflow:auto;">
			<table id="table_pos-rol" class="table table-bordered table-hover table-striped dataTable">
				...
				<tbody> <?php
					$chkmas = MasPositionRol::check_position_Rol($rol);
					$masterdata = MasPositionRol::search($rol);
					...
					foreach ($masterdata as $dataitem) { ?>
						<tr>
							...
							if($dataitem['status'] == 1){ ?>
								<button id="btnsubmitposrol<?php echo $dataitem['id']; ?>" class="btn btn-floating btn-success btn-sm" style="width: 36px; height: 26px;" type="button" title="online" data-id="<?php echo $dataitem['id']; ?>" onclick="changestat(<?php echo $dataitem['id']; ?>)">
									<i class="fa fa-check" aria-hidden="true"></i>
								</button> <?php
							}elseif($dataitem['status'] == 0){ ?>
								<button id="btnsubmitposrol<?php echo $dataitem['id']; ?>" class="btn btn-floating btn-danger btn-sm" style="width: 36px; height: 26px;" type="button" title="offline" data-id="<?php echo $dataitem['id']; ?>" onclick="changestat(<?php echo $dataitem['id']; ?>)">
	...
	<script type="text/javascript">
		...
		function changestat(id) {
			...
			$.ajax({
				type: "POST",
				url:"<?php echo Yii::app()->createAbsoluteUrl("owner/savechangeposrol"); ?>",
				data: {
	...
	//------------------------------------------------------------------------------------------------
		function changeall() {
			...
			var status = '';
			var btn = "button[id^='btnsubmitposrol']";
			if(chkstatus == 1){
			...
			$.ajax({
				type: "POST",
				url:"<?php echo Yii::app()->createAbsoluteUrl("owner/saveposrolall"); ?>",
				data: {
					'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>',
					'rol' : '<?php echo $rol; ?>',
					'status' : status,
