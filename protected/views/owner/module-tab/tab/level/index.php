...
if($mod==""){ ?>
	...
	<div class="lev-mod">
		<div class="col-sm-12 col-xs-12 contact-info-box" style="margin-top:20px;height:500px;overflow:auto;">
			<table id="table_lev-mod" class="table table-bordered table-hover table-striped dataTable">
				<thead style="background-color:#AED6F1;">
					<tr>
						...
						<th class="example-title" id="ca" style="width:70px;"> <?php
							$chkmas0 = MasLevelMod::check_stat_all($mod);
				...
				<tbody> <?php
					$chkmas = MasLevelMod::check_level_mod($mod);
					$masterdata = MasLevelMod::search($mod);
					...
					foreach ($masterdata as $dataitem) { ?>
						<tr>
							...
							<td class="example-title" style="text-align:center; width:70px;"><?php
								...
								<label class="switch">
									<input type="checkbox" <?php echo $ch; ?> id="btnsubmitlevmod<?php echo $dataitem['id']; ?>" data-id="<?php echo $dataitem['id']; ?>" onclick="changestat(<?php echo $dataitem['id']; ?>)">
									<span class="slider round"></span>
								</label>
							</td>
						</tr> <?php
	...
	<script type="text/javascript">
		...
		function changestat(id) {
			...
			$.ajax({
				type: "POST",
				url:"<?php echo Yii::app()->createAbsoluteUrl("owner/savechangelevmod"); ?>",
				data: {
	...
	//------------------------------------------------------------------------------------------------
		function changeall() {
			var status = '';
			var btn = "input[id^='btnsubmitlevmod']";
			...
			$.ajax({
				type: "POST",
				url:"<?php echo Yii::app()->createAbsoluteUrl("owner/savelevmodall"); ?>",
				data: {
					'YII_CSRF_TOKEN': '<?php echo Yii::app()->request->csrfToken; ?>',
					'mod' : '<?php echo $mod; ?>',
					'status' : status,
